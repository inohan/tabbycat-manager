...
FIELD_NAMES = [
    ...
    "speaker_1_name",
    "speaker_1_email",
    "speaker_1_categories",
    "speaker_2_name",
    "speaker_2_email",
    "speaker_2_categories",
    "speaker_3_name",
    "speaker_3_email",
    "speaker_3_categories",
    "speaker_4_name",
    "speaker_4_email",
    "speaker_4_categories",
    "speaker_5_name",
    "speaker_5_email",
    "speaker_5_categories",
    "speaker_6_name",
    "speaker_6_email",
    "speaker_6_categories",
    "speaker_7_name",
    "speaker_7_email",
    "speaker_7_categories",
    "speaker_8_name",
    "speaker_8_email",
    "speaker_8_categories",
    "speaker_9_name",
    "speaker_9_email",
    "speaker_9_categories",
]
...
class TeamImporterRow(ft.DataRow, AppControl):
    ...
    def __init__(self, data: pd.Series):
        ...
        super().__init__([], selected=True, on_select_changed=on_select)
        self.sync_cells()
    ...
    def sync_cells(self):
        ...
        self.cells = [ft.DataCell(_get_content(col)) for col in self.drow.index]

    def build(self):
        super().build()
        if self.app.tournament._links.teams.find(
            lambda team: team.reference == self.drow.get("reference")
            and (team.institution.code if team.institution else None)
            == (value if not pd.isna(value := self.drow.get("institution")) else None)
        ):
            self.selected = False
            ...
            if any(
                ...
                speakers.append(
                    tc.models.Speaker(
                        ...
                        categories=[
                            self.app.tournament._links.speaker_categories.find(
                                slug=to_snake_case(cat)
                            )
                            for cat in cats
                        ]
                        if (cats := _value(f"speaker_{i}_categories")) is not tc.NULL
                        else [],
                    )
                )
        return tc.models.Team(
            institution=self.app.institutions.find(code=inst)
            if (inst := _value("institution")) is not tc.NULL
            else tc.NULL,
            break_categories=[
                self.app.tournament._links.break_categories.find(
                    slug=to_snake_case(cat)
                )
                for cat in _value("break_categories")
            ]
            if (cats := _value("break_categories")) is not tc.NULL
            else [],
            reference=_value("reference"),
            short_reference=_value("short_reference") or ref[:35]
            if (ref := _value("reference")) is not tc.NULL
            else tc.NULL,
            use_institution_prefix=to_bool(value)
            if (value := _value("use_institution_prefix")) is not tc.NULL
            else tc.NULL,
            speakers=speakers,
        )
...
class TeamImporterPagelet(ft.Pagelet, AppControl):
    ...
    def __init__(self):
        self.file_picker = ft.FilePicker(
            on_result=self.on_result_file_pick, on_upload=self.on_upload_complete
        )
        self.dropdown_sheet_select = ft.Dropdown(label="Select Sheet")
        self.button_sheet_select = ft.ElevatedButton(
            "Select sheet", on_click=self.on_select_sheet
        )
        self.data_table = ft.DataTable(
            columns=[ft.DataColumn(ft.Text("No data"))], show_checkbox_column=True
        )
        self.button_import = ft.ElevatedButton("Import", on_click=self.on_import)
        super().__init__(
            ft.Column(
                [
                    ft.Row(
                        [
                            ft.ElevatedButton(
                                ...
                                on_click=lambda _: self.file_picker.pick_files(
                                    dialog_title="Select a team list file",
    ...
    async def on_open_google_picker(self, e):
        ...
        if result.data is not None:
            if result.data.get("mimeType") == "application/vnd.google-apps.spreadsheet":
                ...
                data: bytes = (
                    service.files().get_media(fileId=result.data.get("id")).execute()
                )
    ...
    def on_upload_complete(self, e: ft.FilePickerUploadEvent):
        if e.progress == 1.0:
            ...
            file_path = os.path.join(
                os.getenv("FLET_ASSETS_DIR"), f"uploads/{e.file_name}"
            )
    ...
    def set_sheet_select(self):
        if self.reader and not self.reader.is_specified:
            self.dropdown_sheet_select.options = [
                ft.DropdownOption(key=sheet, text=sheet) for sheet in self.reader.sheets
            ]
    ...
    def set_team_data(self):
        if not (self.reader and self.reader.is_specified):
            self.data_table.columns = [ft.DataColumn(ft.Text("No data"))]
        ...
        self.data_table.rows = [
            TeamImporterRow(self.reader.data.iloc[i])
            for i in range(len(self.reader.data.index))
        ]
        self.set_sheet_select()
        self.update()
    ...
    async def on_import(self, e):
        selected_rows: list[TeamImporterRow] = [
            row
            for row in self.data_table.rows
            if isinstance(row, TeamImporterRow) and row.selected
        ]

        def get_name(data: pd.Series):
            if to_bool(data.get("use_institution_prefix", False)) and data.get(
                "institution"
            ):
                return f"{data.get('institution')} {data.get('reference')}"
            return data.get("reference")

        institutions_table: set[str] = {
            inst
            for row in selected_rows
            if pd.notna((inst := row.drow.get("institution")))
        }
        break_categories_table: set[str] = {
            bc
            for row in selected_rows
            if row.drow.get("break_categories", None)
            for bc in row.drow.get("break_categories")
        }
        speaker_categories_table: set[str] = {
            sc
            for row in selected_rows
            for i in range(1, 10)
            if (ls_scs := row.drow.get(f"speaker_{i}_categories"))
            for sc in ls_scs
        }
        # Find missing objects
        col = ft.Column([], expand=True, scroll=ft.ScrollMode.AUTO)
        # Missing institutions
        missing_institutions = [
            inst
            for inst in institutions_table
            if self.app.institutions.find(code=inst) is None
        ]
        if missing_institutions:
            col.controls.append(
                ft.ExpansionTile(
                    ...
                    controls=[
                        ft.ListTile(ft.Text(inst)) for inst in missing_institutions
                    ],
                    collapsed_text_color=ft.Colors.RED,
                    text_color=ft.Colors.RED,
                    subtitle=ft.Text(
                        f"{len(missing_institutions)} institutions will be created automatically"
                    ),
                )
            )
        # Missing break categories
        missing_break_categories = [
            cat
            for cat in break_categories_table
            if self.app.tournament._links.break_categories.find(slug=to_snake_case(cat))
            is None
        ]
        if missing_break_categories:
            col.controls.append(
                ft.ExpansionTile(
                    ...
                    controls=[
                        ft.ListTile(ft.Text(bc)) for bc in missing_break_categories
                    ],
                    collapsed_text_color=ft.Colors.RED,
                    text_color=ft.Colors.RED,
                    subtitle=ft.Text(
                        f"{len(missing_break_categories)} break categories will be created automatically"
                    ),
                )
            )
        # Missing speaker categories
        missing_speaker_categories = [
            cat
            for cat in speaker_categories_table
            if self.app.tournament._links.speaker_categories.find(
                slug=to_snake_case(cat)
            )
            is None
        ]
        if missing_speaker_categories:
            col.controls.append(
                ft.ExpansionTile(
                    ...
                    controls=[
                        ft.ListTile(ft.Text(value=sc))
                        for sc in missing_speaker_categories
                    ],
                    collapsed_text_color=ft.Colors.RED,
                    text_color=ft.Colors.RED,
                    subtitle=ft.Text(
                        f"{len(missing_speaker_categories)} speaker categories will be created automatically"
                    ),
                )
            )
        # Teams to create
        col.controls.append(
            ft.ExpansionTile(
                ...
                controls=[
                    ft.ListTile(
                        ft.Text(f"{try_string(lambda: get_name(row.drow), "No name")}")
                    )
                    for row in selected_rows
                ],
        ...
        bc_seq = generate_seq(
            max(bc.seq for bc in self.app.tournament._links.break_categories)
            if len(self.app.tournament._links.break_categories)
            else 0
        )
        sc_seq = generate_seq(
            max(sc.seq for sc in self.app.tournament._links.speaker_categories)
            if len(self.app.tournament._links.speaker_categories)
            else 0
        )
            ...
            **{
                f'institution "{inst}"': asyncio.create_task(
                    self.app.tournament.create(
                        tc.models.Institution(name=inst, code=inst)
                    )
                )
                ...
                f'speaker category "{sc}"': asyncio.create_task(
                    self.app.tournament.create(
                        tc.models.SpeakerCategory(
                            name=sc, slug=to_snake_case(sc), seq=next(sc_seq)
                        )
                    )
                )
                for sc in missing_speaker_categories
        ...
        # Create teams
        tasks = {
            get_name(row.drow): asyncio.create_task(self.app.tournament.create(obj))
            for row in selected_rows
        ...
        await asyncio.gather(self.app.update_teams(), self.app.update_speakers())
        self.page.open(
            ft.SnackBar(
